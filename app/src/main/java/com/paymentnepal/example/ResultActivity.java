...
public class ResultActivity extends AppCompatActivity implements AlbaResultReceiver.Receiver  {
    ...
    protected void onResume() {
        ...
        if (resultReceiver == null) {
            ...
            PaymentNepalIntentService.startPayment(
                    this,
                    resultReceiver,
    ...
    private void onStatusInitFinished(Bundle data) {
        ...
        transactionId = data.getInt(PaymentNepalIntentService.DATA_TRANSACTION_ID);
        sessionKey = data.getString(PaymentNepalIntentService.DATA_SESSION_KEY);
        ...
        String termUrl = String.format(acquireUrl,
                resources.getInteger(R.integer.paymentnepal_service_id),
                transactionId);
        ...
        runnable = new Runnable() {
            public void run() {
                ...
                if (runnable == this) {
                        PaymentNepalIntentService.startCheckStatus(
    ...
    public void onStatusCheckFinished(Bundle data) {
        final TextView statusField = (TextView)findViewById(R.id.status);
        final int transactionId = data.getInt(PaymentNepalIntentService.DATA_TRANSACTION_ID);
        Resources resources = getResources();
        String transactionStatus = data.getString(PaymentNepalIntentService.DATA_STATUS);
    ...
    public void onReceiveResult(int resultCode, Bundle data) {
        ...
        switch (resultCode) {
            case PaymentNepalIntentService.STATUS_INIT_ERROR:
                statusField.setText(data.getString("ERROR_MESSAGE"));
                break;
            case PaymentNepalIntentService.STATUS_INIT_FINISHED:
                onStatusInitFinished(data);
                break;
            case PaymentNepalIntentService.STATUS_CHECK_FINISHED:
